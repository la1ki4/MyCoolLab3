...
singlyLinkedList<T>::~singlyLinkedList()
{
    ...
    std::cout << "\n\n¬ызван декстркутор, список очищен!" << std::endl;
}
...
int main()
{
    ...
    {
            //ѕока файл не дошЄл до конца
            while (!file.eof())
            {
                //—оздаЄм временную переменную
                ParticipantsOfTheCompetition tmp;

                //—читываем из файла значени€ во временную переменную
                file >> tmp;

                //≈сли достигнут конец файла, завершаем работу цикла
                if (file.eof())
                {
                    break;
                }
                //«аписываем участника в список
                List.push_back(tmp);
            }
    }
    file.close();
